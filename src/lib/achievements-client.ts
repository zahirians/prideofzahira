...
export async function getPublishedAchievementsClient(
  ...
  if (filters.achievement_type) query = query.eq("achievement_type", filters.achievement_type);
  if (filters.gender) query = query.eq("students.gender", filters.gender);

  const { data: achievements, error } = await query.order("year", { ascending: false });
...
export async function getAchievementByIdClient(
  id: string
